    ...
    <section>
      ...
      {step === "otp" && (
        <form onSubmit={handleSubmit(verifyOtp)} className="space-y-6">
          ...
          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
            <Button onClick={() => setStep("otp")} type="submit" className="w-full bg-gradient-to-r from-emerald-500 to-blue-500 hover:from-emerald-600 hover:to-blue-600 text-white font-bold text-lg py-6 rounded-2xl group cursor-pointer shadow-2xl hover:shadow-emerald-500/25 transition-all duration-300" size="lg" disabled={isSubmitting}>
              {isSubmitting ? (
                <>
                  <div className="w-5 h-5 border-2 border-current border-t-transparent rounded-full animate-spin ml-2"></div>
                  در حال ایجاد حساب...
